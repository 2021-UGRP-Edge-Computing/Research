...
def sendImage(sock, buff,img_index,img_number): # Send (Communication) Core
...
def main():
    image_num=int(input("Input image number: ")) #1000
    time_interval=float(input("Input init time interval size:(float) ")) # 1
    increase_t=float(input("Input time slot increasing size:(float) ")) # 5
    test_num = int(input("Input how many experimnet you perform with increasing time interval:( at least 1) ")) # 6
    sleep_time2=int(input("How much time let nano be free between each experiment: (int)")) # 10
    cpulim = int(input("initial cpu temperature:"))   # 44
    gpulim = int(input("initial gpu temperature:"))   # 42
    ...
    for i in range(test_num):
        ...
        while(True):
            ...
            if context:
                ...
                end_time=time.time()
                context = False
                instant_cap = int(val.value) - history
                ...
                s.connect((host, port))
                print("[+] Connected.")
                sendImage(s, BUFFER_SIZE,val,task_size)
                # Receive Result
            ...
            instant_fps.append((int(val.value) - history)/(end_time - prev_time))
        ...
        instant_ff = open("lab/result1/accident_fps_list" + str(i+1)+".txt",'w')
        for j in instant_fps:
            instant_ff.write(str(j)+" ")
        instant_ff.close()

        instant_tf = open("lab/result1/accident_time_list" + str(i+1)+".txt",'w')
        for j in instant_time:
            instant_tf.write(str(j)+" ")
        instant_tf.close()

        
    jetson.close()     
    tslist_file = open("lab/result1/Time_interval_list.txt",'w')
    ...
    fpslist_file = open("lab/result1/Fps_list.txt",'w')
