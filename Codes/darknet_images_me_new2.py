...
def main():
    ...
    time_interval = float(input("Input init time interval size:(float) 35")) # 1
    increase_t = float(input("Input time slot increasing size:(float) 3")) # 4
    test_num = int(input("Input how many experimnet you perform with increasing time interval:( at least 1) 5")) # 20
    sleep_time2 = int(input("How much time let nano take a rest to match temperature restriction: (int) 30")) # 30
    heating_time = int(input("input Heating time to match the cpu,gpu temperature limit: 30")) # 60
    heat_num = int(input("input how many experimnet you perform with increasing cpu, gpu heating:2")) # 10
    increase_h = float(input("Input inital cpu,gpt temperature increasing size:(float) 3")) # 2.5
    cpulim = int(input("initial minimal cpu temperature:51"))   # 45
    gpulim = int(input("initial minimal gpu temperature:48"))   # 43
    packet_size = float(input("Percentage that you expect from your partner's capablity for task: (100%) "))/100 # 100
    ...
    with open("lab/condition.txt",'w') as f:
        ...
        "Input inital cpu,gpt temperature increasing size:(float)  "+ str(increase_h)+"\n"+
    ...
    for j in range(heat_num):
        ...
        for i in range(test_num):
            ...
            ctc=int(subprocess.check_output(["cat","/sys/devices/virtual/thermal/thermal_zone1/temp"],universal_newlines=True)[:-1])/1000
            ctg=int(subprocess.check_output(["cat","/sys/devices/virtual/thermal/thermal_zone2/temp"],universal_newlines=True)[:-1])/1000
            ...
            bool1=ctc > cpulim + j*increase_h + tol
            bool2=ctg > gpulim + j*increase_h + tol
            bool3=ctc < cpulim + j*increase_h - tol
            bool4=ctg < gpulim + j*increase_h - tol
            ...
            while(processed_imagenum < image_num):
                if context:
                    ...
                    end_time = time.time()
                    instant_cap = processed_imagenum - history
                    #instant_fps.append((instant_cap/(end_time - prev_time)))
                    ...
                    prev_time = time.time()
                    task_size=1
                    if int(instant_cap *packet_size)+history <image_num:
                        task_size = int(instant_cap *packet_size)
                    else:
            ...
            #instant_ff = open("lab/result6/instant_fps_list"+ str(j) + "_" + str(i)+".txt",'w')
            #for k in instant_fps:
            #    instant_ff.write(str(k)+" ")
            #instant_ff.close()

            #instant_tf = open("lab/result6/instant_time_list" + str(j) + "_" + str(i)+".txt",'w')
        ...
        tslist_file = open("lab/result10/Time_interval_list"+str(j)+".txt",'w')
        for m in time_interval_list:
            tslist_file.write(str(m)+" ")
        tslist_file.close()

        fpslist_file = open("lab/result10/Fps_list"+str(j)+".txt",'w')
